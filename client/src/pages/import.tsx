...
export default function Import() {
  ...
  const importMutation = useMutation({
    mutationFn: async (file: File) => {
      ...
      const response = await apiRequest("POST", "/api/import/tv-time", formData)
    ...
    <div className="space-y-8 max-w-4xl">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            Import your watch history from TV Time
          </p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Upload your TV Time export file to automatically import your shows
            and watch progress
          </CardDescription>
        </CardHeader>
            ...
            <div className="space-y-4 pt-4 border-t">
              ...
              {importResult.unmatched > 0 && importResult.unmatchedShows && (
                <Alert variant="destructive">
                  ...
                  <AlertDescription>
                    <p className="mb-2">
                      The following shows could not be matched automatically:
                    </p>
                    <ul className="list-disc list-inside space-y-1 text-sm">
                      {importResult.unmatchedShows
                        .slice(0, 10)
                        .map((show, idx) => (
                          <li key={idx}>{show}</li>
                        ))}
                      {importResult.unmatchedShows.length > 10 && (
                        <li>
                          And {importResult.unmatchedShows.length - 10} more...
                        </li>
                      )}
                    </ul>
                    <p className="mt-2 text-sm">
                      You can manually search and add these shows from the
                      Search page.
                    </p>
                  </AlertDescription>
                </Alert>
